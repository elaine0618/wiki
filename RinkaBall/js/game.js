...
Constants = {
  ...
  ASSETS: {
    ...
    btn_one: './images/btn_one.png',
    btn_options: './images/btn_options.png',
    btn_two: './images/btn_two.png',
    btn_a: './images/btn_a.png',
    btn_b: './images/btn_b.png',
    pole: './images/pole.png',
    blank_point: './images/blank_point.png',
    "return": './images/return.png',
    score_a: './images/score_a.png',
    score_b: './images/score_b.png',
    back_arrow: './images/back_arrow.png',
    options: './images/options.png',
    btn_wiki: './images/btn_wiki.png',
  }
};
...
Slime = (function(_super) {
  ...
  Slime.prototype.handleInput = function(input, accelerate) {
    ...
    if (input.up(pNum)) {
      if (this.y >= Constants.BASE_HEIGHT - Constants.BOTTOM_player - this.height) {
...
World = (function() {
  ...
  function World(width, height, input) {
    ...
    this.pole = new Sprite(this.width / 2 - Constants.POLE_WIDTH / 2, this.height - Constants.BOTTOM_player - Constants.POLE_HEIGHT - 1, Constants.POLE_WIDTH, Constants.POLE_HEIGHT);
  ...
  World.prototype.reset = function(servingPlayer) {
	...
	//每局生成的栏杆位置
    this.pole.setPosition(this.width / 2 - 4, this.height - 90 - 64 - 1, 8, 64);
    this.p1.velocity = {
      x: 0,
      y: 0
    };
  ...
  World.prototype.step = function(interval, dontIncrementClock) {
    ...
    this.boundsCheck();
	//玩家位置
    if (this.p1.y + this.p1.height > this.height - Constants.BOTTOM_player) {
      this.p1.y = this.height - Constants.BOTTOM_player - this.p1.height;
      this.p1.velocity.y = Math.min(this.p1.velocity.y, 0);
    }
    if (this.p2.y + this.p2.height > this.height - Constants.BOTTOM_player) {
      this.p2.y = this.height - Constants.BOTTOM_player - this.p2.height;
...
MenuScene = (function(_super) {
  ...
  function MenuScene() {
	...
	//主界面按钮
    this.buttons = {
      returnWiki: new Button((this.center.x - btnWidth) / 2, dy + yOffset, btnWidth, btnHeight, loader.getAsset('btn_a'), loader.getAsset('btn_b'), this),
      ...
      twoPlayer: new Button(this.center.x + (this.center.x - btnWidth) / 2, dy, btnWidth, btnHeight, loader.getAsset('btn_a'), loader.getAsset('btn_b'), this)
    };
    this.labels = [];
    labelImgs = ['btn_two', 'btn_options', 'btn_one', 'btn_wiki'];
    _ref = ['returnWiki', 'onePlayer', 'options', 'twoPlayer'];
    _fn = function(btn) {
...
SlimeVolleyball = (function(_super) {
  ...
  SlimeVolleyball.prototype.init = function(dontOverrideInput) {
	...
	//进球评价语
    this.failMsgs = ['凛凛凛个憋~', 'Yellow哒呦!', 'Nya nya nya~', '饅頭!', 'バカバカしい!', '*** 凛ちゃんの勝ち ***'];
    this.winMsgs = ['Yahoo~', 'Fightだよ!', '果果酱大胜利!', 'ほのほのほ~', 'Wow~', '*** 穂乃果の勝ち ***'];
  ...
  SlimeVolleyball.prototype.draw = function() {
	...
	//看板文字大小设置
    if (this.displayMsg) {
      this.ctx.font = 'bold 24px ' + Constants.MSG_FONT;
      this.ctx.fillStyle = '#ffffff';
      this.ctx.textAlign = 'center';
      msgs = this.displayMsg.split("\n");
      this.ctx.fillText(msgs[0], this.width / 2, 85);
      if (msgs.length > 1) {
        this.ctx.font = 'bold 21px ' + Constants.MSG_FONT;
